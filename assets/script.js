    ...
    getProducts().then(data => {
        ...
        data.forEach(singleData => {
            ...
            function addLocalProduct() {
                ...
                const productInfo = getLocalInfo();

                products.forEach(function (product) {
                    if (product.product_id === singleData['id']) {
                        let index = products.findIndex((n => n.product_id == product.product_id))
                        products[index].qty = currentAmountItems.value;
                        products[index].item_price = singleData['price'];
                        products[index].item_total = singleData['price'] * products[index].qty;
                        setLocal(products);
                ...
                if (!isProductInCart) {
                    products.push({
                        ...
                        'qty': currentAmountItems.value,
                        'item_price' : singleData['price'],
                        'item_total': singleData['price'] * currentAmountItems.value,
                    })
                    setLocal(products);
                }
                ...
                productInfo.forEach(function (product) {
                    if (product.product_id === singleData['id']) {
                        let index = productInfo.findIndex((n => n.product_id == product.product_id))
                        productInfo[index].qty = currentAmountItems.value;
                        productInfo[index].item_price = singleData['price'];
                        productInfo[index].item_total = singleData['price'] * productInfo[index].qty;
                        setLocalInfo(productInfo);
                        isProductInCart = true;
                        if (product.qty === 0) {
                            removeLocalProduct();
                        }
